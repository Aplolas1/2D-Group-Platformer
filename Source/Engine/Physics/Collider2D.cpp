...
/*This is meant to be part of the player collides with tile map functionality, but it doesn't function correctly

bool Collider2D::CollidesWith(TileMap * pTileMap)
{
	if (pTileMap->m_CollisionRect.left <= (pTileMap->m_CollisionRect.left + pTileMap->m_CollisionRect.width) && (pTileMap->m_CollisionRect.left + pTileMap->m_CollisionRect.width) >= pTileMap->m_CollisionRect.left &&
		pTileMap->m_CollisionRect.top <= (pTileMap->m_CollisionRect.top + pTileMap->m_CollisionRect.height) && (pTileMap->m_CollisionRect.top + pTileMap->m_CollisionRect.height) >= pTileMap->m_CollisionRect.top)
	{
		return true;
	}
